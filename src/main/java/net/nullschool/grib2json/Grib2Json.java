...
public final class Grib2Json {
    ...
    private void write(RandomAccessFile raf, Grib2Input input, Options options) throws IOException {
        ...
        if(options.getFilterLo1() == null || options.getFilterLa1() == null || options.getFilterLo2() == null ||
                options.getFilterLa2() == null){
            ...
            for (Grib2Record record : records) {
                GribRecordWriterWithLoLa rw = new GribRecordWriterWithLoLa(jg, record, options);
                if (rw.isSelected()) {
                    jg.writeStartObject();
                    rw.writeHeader();
                    if (options.getPrintData()) {
